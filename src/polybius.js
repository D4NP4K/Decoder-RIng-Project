...
const polybiusModule = (function () {
  ...
  function polybius(input, encode = true) {
    if(encode){ //if you are encoding  
      return input.split('').map(letter => { //split the input and map the individual letter then return the number value from the object key associated with that letter
      ...
      return input.match(/[0-9]{2}|\s/g).map(number => { //it can be a number 0-9, and there can be 2 of them or a space
